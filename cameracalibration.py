...
chessboard_size = (7, 7)  # 8 squares means 7 internal corners
...
def calibrate_camera(images, frame_size):
    ...
    if len(objpoints) > 0 and len(imgpoints) > 0:
        ...
        messagebox.showinfo("Calibration Results", f"Focal Lengths (fx, fy): {camera_matrix[0, 0]}, {camera_matrix[1, 1]}\nPrincipal Point Coordinates (cx, cy): {camera_matrix[0, 2]}, {camera_matrix[1, 2]}")
    else:
        print("No valid object points or image points were collected for calibration.")
        messagebox.showwarning("Calibration Warning", "No valid object points or image points were collected for calibration.")
...
def process_images(image_files):
    if image_files:
        ...
        messagebox.showwarning("Processing Warning", "No images found in the specified directory.")

def process_videos(video_files):
    if not video_files:
        print("No video files found in the specified directory.")
        messagebox.showwarning("Processing Warning", "No video files found in the specified directory.")
    else:
        for idx, video_file in enumerate(video_files, start=1):
            ...
            frames_collected = 0
            max_frames = 20  # Maximum number of frames to collect
...
def select_path():
    ...
    if not input_path:
        print("Error: Please provide a valid input path.")
        messagebox.showerror("Input Path Error", "Please provide a valid input path.")
        return

    # Load images or videos from the directory
    image_files = glob.glob(os.path.join(input_path, '*.jpg'))
    video_files = glob.glob(os.path.join(input_path, '*.mov')) + glob.glob(os.path.join(input_path, '*.vlc'))

    print("Image files:", image_files)  # Debugging print statement
    print("Video files:", video_files)  # Debugging print statement
...
def main():
    ...
    Label(root).pack(side=TOP,pady=10)
    w = Label(root, text="I receive an image or a video!")
    x = Label(root, text="You get camera intrinsic features!")

    btn_select_path = tk.Button(root, text="Select Input Path", command=select_path)
    w.pack()
    x.pack()
    btn_select_path.pack(pady=20)
    root.resizable(0,0)
    root.mainloop()
